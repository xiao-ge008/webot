...
import { requestJson, requestOpenFangJson, requestSse, requestWebSocket } from '@/services/transport';
import { getAgentSkillAssignments, getManagementAgentDetail, listManagementA2aAgents, listManagementAgents } from '@/services/management-client';
...
function parseCollaborationConfigFromText(rawText: string): CollaborationConfigPayload | null {
  ...
  try {
    ...
    const selectedWorkers = Array.isArray(parsed.selectedWorkers)
      ? parsed.selectedWorkers.filter((item): item is string => typeof item === 'string' && item.trim().length > 0)
      : [];
    return {
...
function extractUiRawText(raw: string): string {
  ...
  const idx = text.indexOf('<UI_JSON>');
  return idx >= 0 ? text.slice(idx).trim() : '';
}

function extractLeadingSpecStreamPatch(buffer: string): { consumed: string; remaining: string; patches: string[] } {
...
function extractJsonBlocks(text: string): Array<{ raw: string; value: unknown }> {
...
function looksLikeGenUiSpec(candidate: unknown): boolean {
  ...
  if (typeof obj.root === 'string' && obj.elements && typeof obj.elements === 'object') {
    ...
    return elements.some((el) => typeof el === 'object' && el !== null && typeof (el as any).type === 'string');
  }
...
function looksLikeGenUiPatch(candidate: unknown): boolean {
  ...
  return ['add', 'remove', 'replace', 'move', 'copy', 'test'].includes(obj.op) && obj.path.startsWith('/');
}

const NON_UI_EVENT_TYPES = new Set(['typing', 'phase', 'response', 'done', 'error', 'tool_result', 'tool_use']);
...
function parseSpecFromText(text: string): unknown | undefined {
  const uiTagMatches = Array.from(text.matchAll(/<UI_JSON>\s*([\s\S]*?)\s*<\/UI_JSON>/ig));
  if (uiTagMatches.length > 0) {
    const taggedSpecs = uiTagMatches
      .map((match) => normalizeGenUiSpec(parseJsonSafely<unknown>((match[1] || '').trim())))
      .filter((item): item is unknown => item !== undefined);
  ...
  const patchLines = blocks.filter((item) => looksLikeGenUiPatch(item.value)).map((item) => item.raw.trim());
...
function parseSpecFromEventPayload(payload: Record<string, unknown>): unknown | undefined {
  const candidates = [payload.spec, payload.ui, payload.schema, payload.component, payload.render];
  ...
  const textCandidate = toStringValue(payload.text) || toStringValue(payload.content) || toStringValue(payload.message);
...
async function buildCollaborationAgentHint(currentAgentId: string): Promise<string> {
  ...
  try {
    ...
    const parsedConfig = currentAgent.collaboration || parseCollaborationConfigFromText(currentAgent.system_prompt || '');
    ...
    if (!dispatchEnabled) {
      ...
      const selectedWorkerKeys = normalizeCollaborationWorkerKeys(parsedConfig?.selectedWorkers || []);
      const hasA2aWorkers = selectedWorkerKeys.some((key) => key.startsWith('a2a:'));
...
function parseTextToolCallPayload(text: string): ParsedTextToolCall | null {
  ...
  for (const line of lines.slice(1)) {
    const argNameMatch = line.match(/^<arg_name>\s*([^<]+)\s*<\/arg_name>\s*$/i);
    if (argNameMatch?.[1]) {
      pendingArgName = argNameMatch[1].trim();
      continue;
    }

    const argValueMatch = line.match(/^<arg_value>\s*([\s\S]*?)\s*<\/arg_value>\s*$/i);
    if (argValueMatch) {
...
async function runTextToolCallFallback(text: string): Promise<{ text: string; tool: string; input: Record<string, unknown> } | null> {
...
async function buildUiEnvironmentSystemPrompt(input: AgentChatInput): Promise<string> {
  ...
  try {
    ...
    const skillNames = assignments.assigned.filter((item): item is string => typeof item === 'string' && item.trim().length > 0);
    if (skillNames.length > 0) {
      components = await listAvailableSkillComponents(input.agentId, skillNames);
    }
  } catch {
    components = [];
  }
  ...
  const injectedManifestContexts = manifestContexts
    .map((item, index) => {
      ...
      return [
        ...
        item.propsSchema ? `propsSchema: ${JSON.stringify(item.propsSchema)}` : '',
  ...
  return [
    ...
    'Common built-in components you should actively consider when useful: `OptionSelector`, `LineChartCard`, `BarChartCard`, `AreaChartCard`, `PieChartCard`, `ImageCover`, `ImageAlbum`, `ImageCarousel`, `WebViewCard`, `MarkdownPreviewCard`, `OfficePreviewCard`.',
...
function shouldInjectUiRenderHint(input: AgentChatInput): boolean {
  ...
  if (renderMode === CHAT_RENDER_MODES.jsonRender || renderMode === CHAT_RENDER_MODES.gui) {
    return true;
  }
  if (renderMode === CHAT_RENDER_MODES.markdown || renderMode === CHAT_RENDER_MODES.plainText) {
    return false;
  }
...
async function buildOutgoingMessage(input: AgentChatInput): Promise<string> {
  ...
  const systemPreamble = typeof input.systemPreamble === 'string' ? input.systemPreamble.trim() : '';
  return [
...
export function withChatRenderContext(
  ...
  return {
    ...
    renderMode: overrides?.renderMode ?? input.renderMode ?? CHAT_RENDER_MODES.jsonRender,
  };
}
...
async function loadSkillPromptContext(componentName: string, agentId?: string): Promise<string> {
...
async function listAvailableSkillComponents(agentId: string, skillNames: string[]): Promise<string[]> {
  const normalizedSkills = skillNames.map((item) => item.trim()).filter(Boolean).sort();
  ...
  try {
    ...
    const components = Array.isArray(payload?.components)
      ? payload.components.filter((item): item is string => typeof item === 'string' && item.trim().length > 0)
      : [];
...
async function loadSkillComponentManifest(componentName: string, agentId?: string): Promise<{ description: string; propsSchema: unknown; example: unknown } | null> {
  const normalized = componentName.trim();
  ...
  try {
    ...
    const payload = await invoke.invoke('load_skill_component_manifest', {
      componentName: normalized,
      agentId: agentId ?? null,
    }) as { description?: unknown; props_schema?: unknown; propsSchema?: unknown; example?: unknown };
    const manifest = {
...
function mapAgentToProfile(agent: OpenFangAgentSummary | OpenFangAgentDetail): AgentProfile {
  const createdAt = toStringValue(agent.created_at, new Date().toISOString());
  const provider =
    isRecord(agent) && isRecord(agent.model)
      ? toStringValue(agent.model.provider, toStringValue(agent.model_provider, 'unknown'))
      : toStringValue(agent.model_provider, 'unknown');
  const model =
    isRecord(agent) && isRecord(agent.model)
      ? toStringValue(agent.model.model, toStringValue(agent.model_name, 'unknown'))
  ...
  return {
    ...
    name: toStringValue(
      agent.nickname,
      toStringValue(agent.name, toStringValue(agent.english_name, toStringValue(agent.id, '鏈懡鍚嶆櫤鑳戒綋'))),
    ),
    title: toStringValue(agent.profile),
    tags: Array.isArray(agent.tags) ? agent.tags.filter((item): item is string => typeof item === 'string') : [],
...
function mapOpenFangCronJobToAgentTask(job: OpenFangCronJob): AgentTask {
  const scheduleKindRaw = toStringValue(job.schedule?.kind, 'cron').toLowerCase();
  const scheduleKind: AgentTask['scheduleKind'] =
    scheduleKindRaw === 'every' || scheduleKindRaw === 'at' ? scheduleKindRaw : 'cron';

  const actionKind = toStringValue(job.action?.kind).toLowerCase();
  const jobType: AgentTask['jobType'] = actionKind === 'system_event' ? 'shell' : 'agent';
  const prompt = toStringValue(job.action?.message, toStringValue(job.action?.text));
  ...
  const isChatTaskPrompt = chatPromptMarkers.filter((marker) => normalizedPrompt.includes(marker)).length >= 2;
  const sourceType: AgentTask['sourceType'] = isChatTaskName || isChatTaskPrompt ? 'chat' : 'custom';
  ...
  return {
    ...
    sourceType,
    scheduleKind,
    scheduleExpression: scheduleKind === 'cron' ? toStringValue(job.schedule?.expr) : undefined,
    ...
    lastStatus: toStringValue(job.last_status, toStringValue(job.status, 'idle')),
  };
}
...
async function invokeIpc<TResponse>(channel: string, payload?: unknown): Promise<TResponse> {
  ...
  const result = (await ipc.invoke(channel, payload)) as SettingsApiResult<TResponse> | TResponse;
...
export async function getAgent(agentId: string): Promise<AgentProfile> {
  const data = await requestJson<unknown>(`/api/management/agents/${encodeURIComponent(agentId)}`);
  return mapAgentToProfile((data ?? {}) as OpenFangAgentDetail);
}

export async function listAgents(_input?: ListAgentsInput): Promise<readonly AgentProfile[]> {
...
export async function stopAgent(input: StopAgentInput): Promise<StopAgentResult> {
  try {
    await requestJson<unknown>(`/api/management/agents/${encodeURIComponent(input.agentId)}/stop`, {
      method: 'POST',
      body: {},
    });
...
export async function getAgentStatus(agentId: string): Promise<AgentRuntimeStatus> {
  const detail = await requestJson<unknown>(`/api/management/agents/${encodeURIComponent(agentId)}`);
...
export async function getAgentLogTail(input: GetAgentLogTailInput): Promise<AgentLogTail> {
  try {
    return await invokeIpc<AgentLogTail>(AGENT_IPC_CHANNELS.agentLogTail, input);
  } catch {
    return {
...
export async function getAgentCollaborationEvents(
  ...
  try {
    return await invokeIpc<readonly AgentCollaborationEvent[]>(AGENT_IPC_CHANNELS.agentCollaborationEvents, input);
  } catch {
    return [];
  }
}
  ...
  try {
    ...
    if (!isTauriRuntime() || sessionId || sessionLabel) {
      await requestSse(
        ...
        (frame) => {
          ...
          const eventName = toStringValue(frame.event, 'message').trim().toLowerCase();
          ...
          if (eventName === 'chunk' || eventName === 'message') {
            const textDelta = payload ? toStringValue(payload.content) : frame.data;
            if (textDelta) {
              fullText += textDelta;
              emitChunk({
                requestId,
          ...
          if (eventName === 'spec_patch') {
            const patchText = payload ? toStringValue(payload.patch, toStringValue(payload.content)) : frame.data;
            if (patchText) {
              emitChunk({
                requestId,
                kind: 'patch',
          ...
          if (
            ...
            if (eventName === 'phase' && payload && toStringValue(payload.phase).trim().toLowerCase() === 'done') {
              sawPhaseDone = true;
            }
            if (eventName === 'typing' && payload && toStringValue(payload.state).trim().toLowerCase() === 'stop') {
              sawTypingStop = true;
            }
          ...
          if (eventName === 'done') {
            ...
            clearCompletionAbortTimer();
            donePayload = payload ?? {};
            const responseText = payload ? toStringValue(payload.content).trim() : '';
            if (responseText) {
          ...
          if (eventName === 'error') {
            ...
            streamErrorMessage = payload
              ? toStringValue(payload.error, toStringValue(payload.message, toStringValue(payload.content)))
              : frame.data;
            return;
          }
      ...
      try {
        await requestWebSocket(
          ...
          (frame) => {
            ...
            if (type === 'text_delta') {
              ...
              const looksLikePatchStart = trimmedDelta.startsWith('{') || patchDeltaBuffer.trimStart().startsWith('{');
            ...
            if (type === 'typing' || type === 'phase') {
              if (type === 'phase' && toStringValue(payload.phase).trim().toLowerCase() === 'done') {
                sawPhaseDone = true;
              }
              if (type === 'typing' && toStringValue(payload.state).trim().toLowerCase() === 'stop') {
                sawTypingStop = true;
              }
              emitChunk({
                requestId,
        ...
        const abortedForCompletion = controller.signal.aborted && sawPhaseDone && sawTypingStop && !sawTerminalEvent;
        const abortedByIdleTimeout = controller.signal.aborted && idleTimeoutTriggered && !sawTerminalEvent;
        const abortedByMaxTimeout = controller.signal.aborted && maxTimeoutTriggered && !sawTerminalEvent;
        if (!abortedForCompletion && !abortedByIdleTimeout && !abortedByMaxTimeout) {
          emitChunk({
            requestId,
            kind: 'log',
            value: wsError instanceof Error ? wsError.message : 'WebSocket 杩炴帴澶辫触',
            event: 'phase',
          });
          ...
          fullText = toStringValue(response.response, toStringValue(response.content));
    ...
    if (!sawTerminalEvent && (idleTimeoutTriggered || maxTimeoutTriggered)) {
      ...
      donePayload = {
        type: 'response_fallback',
        content: fullText,
        fallback: idleTimeoutTriggered ? 'stream_idle_timeout' : 'stream_max_timeout',
      };
    }
    ...
    if (!sawToolResultEvent && looksLikeTextToolCallOnly(fullText)) {
      ...
      if (fallback) {
        ...
        try {
          ...
          const retryText = toStringValue(retryResponse.response, toStringValue(retryResponse.content)).trim();
    ...
    emitChunk({
      ...
      meta: {
        rawEvent: 'done',
        rawPayload: JSON.stringify(donePayload ?? { text: fullText, spec: doneSpec ?? null }),
      },
    });
...
export async function sendAgentChat(input: AgentChatInput): Promise<AgentChatResult> {
  if (input.stream) {
    const requestId = input.requestId || createRequestId();
...
export async function cancelAgentChat(input: CancelAgentChatInput): Promise<CancelAgentChatResult> {
  ...
  try {
    await requestJson<unknown>(`/api/management/agents/${encodeURIComponent(agentId)}/stop`, {
      method: 'POST',
      body: {},
    });
    return {
...
export async function getAgentSession(agentId: string): Promise<AgentSessionResult> {
  try {
    const data = await requestJson<unknown>(`/api/chat/${encodeURIComponent(agentId)}/session`);
    if (!isRecord(data)) {
      return {
        success: true,
        messages: [],
...
export async function listAgentTasks(input: AgentTaskListInput): Promise<AgentTaskListResult> {
  try {
    const query = `?agent_id=${encodeURIComponent(input.agentId)}`;
    const data = await requestJson<unknown>(`/api/management/cron/jobs${query}`);
    ...
    return {
      success: false,
      message: error instanceof Error ? error.message : '浠诲姟鍒楄〃鑾峰彇澶辫触',
      tasks: [],
    };
  }
}

function buildCreateCronPayload(input: AgentTaskCreateInput): Record<string, unknown> {
  const timeoutSecs = (() => {
    if (typeof input.timeoutSecs !== 'number' || !Number.isFinite(input.timeoutSecs)) {
      return 180;
    }
  ...
  const normalizeCronExpr = (raw?: string): string => {
    const trimmed = toStringValue(raw).trim().replace(/^[`'"]+|[`'"]+$/g, '').trim();
    if (!trimmed) {
      return '';
    }
  ...
  if (input.scheduleKind === 'cron' && cronExpr.split(' ').length !== 5) {
    throw new Error('CRON 表达式必须是 5 段：minute hour day-of-month month day-of-week');
  }
  ...
  const schedule =
    input.scheduleKind === 'at'
      ...
      : input.scheduleKind === 'every'
        ? {
          kind: 'every',
          every_secs: Math.max(60, Math.round((input.everyMs ?? 60_000) / 1000)),
        }
        : {
          kind: 'cron',
          expr: cronExpr || '* * * * *',
  ...
  const action =
    input.jobType === 'shell'
      ...
      : {
        kind: 'agent_turn',
        message: input.prompt || input.command || input.name || 'scheduled task',
  ...
  const delivery =
    input.deliveryMode === 'announce'
      ? (() => {
        ...
        const target = toStringValue(input.deliveryTarget, fallbackTarget).trim();
        if (channel && target) {
          return {
            kind: 'channel',
            channel,
...
export async function createAgentTask(input: AgentTaskCreateInput): Promise<AgentTaskCreateResult> {
  try {
    ...
    const task = jobId ? listed.tasks.find((item) => item.id === jobId) : listed.tasks[0];
...
export async function deleteAgentTask(input: AgentTaskDeleteInput): Promise<AgentTaskDeleteResult> {
  try {
    await requestJson<unknown>(`/api/management/cron/jobs/${encodeURIComponent(input.taskId)}`, {
      method: 'DELETE',
    });
    return {
      success: true,
...
export async function getAgentTaskProgress(input: AgentTaskProgressInput): Promise<AgentTaskProgressResult> {
  try {
    ...
    const readRunCountHint = (obj: Record<string, unknown>): number | undefined => {
      const directKeys = [
        'run_count',
        'runCount',
        'total_runs',
    ...
    return {
      success: true,
      task,
      runCountHint: typeof runCountHint === 'number' && runCountHint > 0 ? runCountHint : undefined,
      logs,
    };
  } catch (error) {
    return {
      success: false,
      message: error instanceof Error ? error.message : '浠诲姟杩涘害鏌ヨ澶辫触',
      logs: [],
    };
  }
}
...
export function subscribeAgentChatStream(
  ...
  const listener = (payload: unknown) => handler(payload as AgentChatStreamChunk);
  const unsubscribe = emitter?.on?.(AGENT_IPC_CHANNELS.agentChatStream, listener);

...
const TOPIC_SUMMARY_RECENT_MESSAGES: usize = 8;
...
const RECALL_CANDIDATE_LIMIT_MAX: usize = 24;
const EMBEDDING_CIRCUIT_FAILURE_THRESHOLD: u32 = 3;
const EMBEDDING_CIRCUIT_COOLDOWN_SECS: u64 = 90;

#[derive(Debug, Default)]
struct EmbeddingCircuitState {
    consecutive_failures: u32,
    cooldown_until: Option<Instant>,
}

fn embedding_circuit_state() -> &'static Mutex<EmbeddingCircuitState> {
    static STATE: OnceLock<Mutex<EmbeddingCircuitState>> = OnceLock::new();
    STATE.get_or_init(|| Mutex::new(EmbeddingCircuitState::default()))
}

fn embedding_circuit_is_open() -> bool {
    let now = Instant::now();
    let mut state = embedding_circuit_state()
        .lock()
...
fn embedding_circuit_record_success() {
    let mut state = embedding_circuit_state()
        .lock()
        .unwrap_or_else(|e| e.into_inner());
    if state.consecutive_failures > 0 || state.cooldown_until.is_some() {
        debug!("Embedding circuit reset after successful request");
    }
    state.consecutive_failures = 0;
    state.cooldown_until = None;
}

fn embedding_circuit_record_failure(error: &str) {
    let mut state = embedding_circuit_state()
        .lock()
        .unwrap_or_else(|e| e.into_inner());
    ...
    if state.consecutive_failures >= EMBEDDING_CIRCUIT_FAILURE_THRESHOLD {
        ...
        warn!(
            ...
            "Embedding circuit opened after consecutive failures: {error}"
        );
    }
}
...
fn memory_turn_context_from_manifest(manifest: &AgentManifest) -> Option<MemoryTurnContext> {
    manifest
        .metadata
        .get(MANIFEST_MEMORY_TURN_CONTEXT_KEY)
        .and_then(|v| serde_json::from_value::<MemoryTurnContext>(v.clone()).ok())
        .and_then(|ctx| {
            let has_conversation_scope = ctx
                .conversation_scope
                .as_deref()
                .map(str::trim)
                .is_some_and(|v| !v.is_empty());
            let has_participant_scope = ctx
                .participant_scope
                .as_deref()
                .map(str::trim)
                .is_some_and(|v| !v.is_empty());
            if has_conversation_scope || has_participant_scope {
                Some(ctx)
            } else {
                None
            }
...
fn memory_conversation_scope(session: &Session) -> Option<String> {
    session
        .label
        .as_deref()
        .map(str::trim)
...
async fn recall_with_filter(
    ...
    if let Some(query_vec) = query_embedding {
        memory
            .recall_with_embedding_async(user_message, limit, Some(filter), Some(query_vec))
            .await
            .unwrap_or_default()
    } else {
        memory
            .recall(user_message, limit, Some(filter))
            .await
            .unwrap_or_default()
    }
}

fn merge_recall_with_participant_weight(
    personal: Vec<openfang_types::memory::MemoryFragment>,
    shared: Vec<openfang_types::memory::MemoryFragment>,
    limit: usize,
) -> Vec<openfang_types::memory::MemoryFragment> {
    // Score by position in each list, with participant-specific memories getting a boost.
    let mut merged: HashMap<uuid::Uuid, (f32, u8, usize, openfang_types::memory::MemoryFragment)> =
        HashMap::new();

    for (rank, item) in personal.into_iter().enumerate() {
        let score = 1.0 / ((rank + 1) as f32) + PARTICIPANT_RECALL_BOOST;
        merged.insert(item.id.0, (score, 0, rank, item));
    }

    for (rank, item) in shared.into_iter().enumerate() {
        let score = 1.0 / ((rank + 1) as f32);
        let key = item.id.0;
        match merged.get_mut(&key) {
            Some((existing_score, source_priority, existing_rank, existing_item)) => {
            ...
            None => {
                merged.insert(key, (score, 1, rank, item));
            }
        }
    }

    let mut scored = merged.into_values().collect::<Vec<_>>();
    scored.sort_by(|a, b| {
        b.0.total_cmp(&a.0)
...
async fn recall_memories_for_turn(
    ...
    let conversation_scope = memory_conversation_scope(session).or_else(|| {
        memory_turn_context
            .and_then(|ctx| ctx.conversation_scope.clone())
            .map(|v| v.trim().to_string())
            .filter(|v| !v.is_empty())
    });
    let participant_scope = memory_turn_context
    ...
    if let Some(scope) = conversation_scope {
        shared_filter.metadata.insert(
            META_CONVERSATION_SCOPE.to_string(),
            serde_json::Value::String(scope),
        );
    }

    let query_vec = if let Some(emb) = embedding_driver {
        if embedding_circuit_is_open() {
            ...
            match emb.embed_one(recall_source).await {
                ...
                Err(e) => {
                    embedding_circuit_record_failure(&e.to_string());
    ...
    if let Some(participant_scope) = participant_scope {
        ...
        participant_filter.metadata.insert(
            META_PARTICIPANT_SCOPE.to_string(),
            serde_json::Value::String(participant_scope),
        );

        let candidate_limit = (MEMORY_RECALL_LIMIT * RECALL_CANDIDATE_LIMIT_MULTIPLIER)
        ...
        .await;
        merge_recall_with_participant_weight(personal, shared, MEMORY_RECALL_LIMIT)
    } else {
        recall_with_filter(
            memory,
            recall_query,
...
async fn remember_interaction_orchestrated(
    memory: &MemorySubstrate,
    session: &Session,
    user_message: &str,
    final_response: &str,
    ...
    let candidates = build_memory_candidates(user_message, final_response, session);
    let conversation_scope = memory_conversation_scope(session).or_else(|| {
        memory_turn_context
            .and_then(|ctx| ctx.conversation_scope.clone())
            .map(|v| v.trim().to_string())
    ...
    let participant_scope = memory_turn_context
        ...
        .map(|v| v.trim().to_string())
        .filter(|v| !v.is_empty());
    let mut embedding_blocked = embedding_driver.is_some() && embedding_circuit_is_open();
    if embedding_blocked {
        debug!("Embedding circuit open; remember path will fallback to text memory");
    }
    for candidate in candidates {
        ...
        metadata.insert(
            ...
            serde_json::Value::String(candidate.memory_type.clone()),
        );
        metadata.insert(
            "importance".to_string(),
            serde_json::Value::from(candidate.importance as f64),
        );
        metadata.insert(
            "confidence".to_string(),
            serde_json::Value::from(candidate.confidence as f64),
        );
        if let Some(entity_key) = candidate.entity_key.clone() {
            metadata.insert(
                "entity_key".to_string(),
                serde_json::Value::String(entity_key),
            );
        }
        if let Some(days) = candidate.expires_days {
            let expires_at = (Utc::now() + ChronoDuration::days(days)).to_rfc3339();
            metadata.insert(
                "expires_at".to_string(),
                serde_json::Value::String(expires_at),
            );
        }
        if is_personal_memory_type(&candidate.memory_type) {
            if let Some(scope) = participant_scope.clone() {
                metadata.insert(
                    META_PARTICIPANT_SCOPE.to_string(),
                    serde_json::Value::String(scope),
                );
            } else if let Some(scope) = conversation_scope.clone() {
                metadata.insert(
                    META_CONVERSATION_SCOPE.to_string(),
                    serde_json::Value::String(scope),
                );
        ...
        if let Some(emb) = embedding_driver {
            ...
            match emb.embed_one(&candidate.content).await {
                Ok(vec) => {
                    embedding_circuit_record_success();
                    let _ = memory
                ...
                Err(e) => {
                    ...
                    warn!("Embedding remember failed, fallback to text memory: {e}");
                    let _ = memory
                        .remember(
                            session.agent_id,
                            &candidate.content,
...
fn build_memory_candidates(
    ...
    let mut candidates = vec![MemoryWriteCandidate {
        ...
        memory_type: "summary_turn".to_string(),
        importance: 0.55,
        confidence: 0.75,
        entity_key: None,
        expires_days: Some(7),
    ...
    if is_task_state_statement(user_memory_text) {
        candidates.push(MemoryWriteCandidate {
            ...
            importance: 0.9,
            confidence: 0.9,
            entity_key: Some("session.task_state.current".to_string()),
            expires_days: Some(30),
        });
    }
    ...
    if is_fact_statement(user_memory_text) {
        candidates.push(MemoryWriteCandidate {
            ...
            confidence: 0.82,
            entity_key: None,
            expires_days: Some(180),
        });
    }

    if should_emit_topic_summary(session.messages.len()) {
        let topic = build_topic_summary(session, TOPIC_SUMMARY_RECENT_MESSAGES);
        if !topic.is_empty() {
            candidates.push(MemoryWriteCandidate {
                content: topic,
                scope: "long_term".to_string(),
                memory_type: "summary_topic".to_string(),
                importance: 0.7,
                confidence: 0.8,
                entity_key: Some("session.topic.latest".to_string()),
                expires_days: Some(30),
            });
        }
    }
...
fn should_emit_topic_summary(message_count: usize) -> bool {
    message_count >= TOPIC_SUMMARY_TRIGGER_MESSAGES
        && message_count % TOPIC_SUMMARY_TRIGGER_MESSAGES == 0
}

fn build_topic_summary(session: &Session, recent_messages: usize) -> String {
    let mut snippets = Vec::new();
    for msg in session.messages.iter().rev().take(recent_messages).rev() {
        ...
        snippets.push(format!("{role}: {}", truncate_for_memory(&text, 120)));
    }
...
fn is_preference_statement(user_message: &str) -> bool {
    let s = user_message;
    s.contains("请记住")
        || s.contains("记住我")
        || s.contains("我喜欢")
...
fn is_task_state_statement(user_message: &str) -> bool {
    let s = user_message;
    s.contains("我们现在")
        || s.contains("当前任务")
        || s.contains("目标是")
...
fn is_fact_statement(user_message: &str) -> bool {
    let s = user_message;
    s.contains("我是")
        || s.contains("我在")
        || s.contains("我的")
        || s.contains("我用")
        || s.contains("my name is")
        || s.contains("i am")
}

fn detect_preference_entity_key(user_message: &str) -> Option<String> {
    let s = user_message.to_lowercase();
    if s.contains("中文") || s.contains("chinese") {
        return Some("user.preference.language".to_string());
    }
    if s.contains("英文") || s.contains("english") {
        return Some("user.preference.language".to_string());
    }
    if s.contains("简洁") || s.contains("简短") || s.contains("concise") {
        return Some("user.preference.response_style".to_string());
    }
    if s.contains("详细") || s.contains("detail") {
        return Some("user.preference.response_style".to_string());
    }
    None
}
...
pub async fn run_agent_loop(
    ...
    let hand_allowed_env: Vec<String> = manifest
        ...
        .unwrap_or_default();
    let memory_turn_context = memory_turn_context_from_manifest(manifest);

    // Recall relevant memories through orchestrated policy.
    let memories = recall_memories_for_turn(
        memory,
        session,
        user_message,
        embedding_driver,
        memory_turn_context.as_ref(),
    )
    .await;
    ...
    for iteration in 0..max_iterations {
        ...
        match response.stop_reason {
            StopReason::EndTurn | StopReason::StopSequence => {
                ...
                // Remember this interaction using typed-memory orchestration.
                remember_interaction_orchestrated(
                    memory,
                    session,
                    user_message,
                    &final_response,
...
fn recover_text_tool_calls(text: &str, available_tools: &[ToolDefinition]) -> Vec<ToolCall> {
    ...
    let sanitize_text_tool_value = |value: &str| -> String {
        value
            .replace("</arg_value>", "")
            .replace("<arg_value>", "")
            .replace("</arg_name>", "")
    ...
    // Pattern 6: <tool_call>TOOL_NAME followed by multi-line `key: value`
    {
        let lines: Vec<&str> = text.lines().collect();
        let mut index = 0usize;
        while index < lines.len() {
            let line = lines[index].trim();
            if let Some(tool_name) = line.strip_prefix("<tool_call>").map(str::trim) {
                if tool_name.is_empty() || !tool_names.contains(&tool_name) {
                    index += 1;
                    continue;
                }
                ...
                while lookahead < lines.len() {
                    let kv_line = lines[lookahead].trim();
                    if kv_line.is_empty() {
                        lookahead += 1;
                        continue;
                    }
                    if let Some(name) = kv_line
                        .strip_prefix("<arg_name>")
                        .and_then(|v| v.strip_suffix("</arg_name>"))
                        .map(str::trim)
                    {
                        if !name.is_empty() {
                            pending_arg_name = Some(name.to_string());
                            lookahead += 1;
                            continue;
                        }
                    }
                    if let Some(value) = kv_line
                        .strip_prefix("<arg_value>")
                        .and_then(|v| v.strip_suffix("</arg_value>"))
                    {
                        if let Some(arg_name) = pending_arg_name.take() {
                            let cleaned = sanitize_text_tool_value(value);
                            if !cleaned.is_empty() {
                                map.insert(arg_name, serde_json::Value::String(cleaned));
                            }
                        }
                        lookahead += 1;
                        continue;
                    }
                    if kv_line.starts_with('<') && !kv_line.starts_with("</arg_value>") {
                        break;
                    }
                    if let Some((key, value)) = kv_line.split_once(':') {
                        let key = sanitize_text_tool_value(key);
                        let value = sanitize_text_tool_value(value);
                    ...
                    if let Some(arg_name) = pending_arg_name.take() {
                        let value = sanitize_text_tool_value(kv_line);
                        if !value.is_empty() {
                            map.insert(arg_name, serde_json::Value::String(value));
                            lookahead += 1;
                            continue;
                        }
                    }
                ...
                if let Some(arg_name) = pending_arg_name.take() {
                    if !arg_name.is_empty() && !map.contains_key(&arg_name) {
                        // incomplete arg_name without value — ignore
                    }
                }
                ...
                if !map.is_empty() {
                        ...
                        .any(|c| c.name == tool_name && c.input == input)
                    {
                        info!(
                            tool = tool_name,
                            "Recovered text-based tool call (<tool_call> key:value variant) → synthetic ToolUse"
                        );
                        calls.push(ToolCall {
                            id: format!("recovered_{}", uuid::Uuid::new_v4()),
                            name: tool_name.to_string(),
...
mod tests {
    ...
    #[test]
    fn test_recover_tool_call_tag_key_value_format() {
        let tools = vec![ToolDefinition {
            name: "web_search".into(),
        ...
        let text = "<tool_call>web_search\n  query: 深圳天气 2026年3月7日 实时";
        let calls = recover_text_tool_calls(text, &tools);
        assert_eq!(calls.len(), 1);
        assert_eq!(calls[0].name, "web_search");
        assert_eq!(calls[0].input["query"], "深圳天气 2026年3月7日 实时");
    }

    #[test]
    fn test_recover_tool_call_tag_key_value_with_arg_value_suffix() {
        let tools = vec![ToolDefinition {
            name: "web_search".into(),
            description: "Search".into(),
            input_schema: serde_json::json!({
        ...
        let text = "<tool_call>web_search\n  query: 深圳天气 2026年3月7日 今天实时</arg_value>";
        let calls = recover_text_tool_calls(text, &tools);
        assert_eq!(calls.len(), 1);
        assert_eq!(calls[0].name, "web_search");
        assert_eq!(calls[0].input["query"], "深圳天气 2026年3月7日 今天实时");
    }

    #[test]
    fn test_recover_tool_call_tag_xml_arg_format() {
        let tools = vec![ToolDefinition {
            name: "web_search".into(),
            description: "Search".into(),
            input_schema: serde_json::json!({
        ...
        let text = "<tool_call>web_search\n<arg_name>query</arg_name>\n<arg_value>深圳天气 2026年3月7日 今天实时</arg_value>";
        let calls = recover_text_tool_calls(text, &tools);
        assert_eq!(calls.len(), 1);
        assert_eq!(calls[0].name, "web_search");
        assert_eq!(calls[0].input["query"], "深圳天气 2026年3月7日 今天实时");
    ...
    #[tokio::test]
    async fn test_text_tool_call_tag_recovery_streaming_e2e() {
        struct ToolCallTagDriver {
            call_count: AtomicU32,
        }

        #[async_trait]
        impl LlmDriver for ToolCallTagDriver {
            async fn complete(
                &self,
                _request: CompletionRequest,
            ) -> Result<CompletionResponse, LlmError> {
                ...
                if call == 0 {
                    ...
                    Ok(CompletionResponse {
                        content: vec![ContentBlock::Text {
                            text: "根据搜索结果，深圳今天天气晴，温度约 22°C。".to_string(),
                        }],
                        stop_reason: StopReason::EndTurn,
                        tool_calls: vec![],
                        usage: TokenUsage {
        ...
        let manifest = test_manifest();
        let driver: Arc<dyn LlmDriver> = Arc::new(ToolCallTagDriver {
            call_count: AtomicU32::new(0),
        });
        ...
        .await
        .expect("Streaming loop should complete for <tool_call> variant");

        assert!(!result.response.contains("<tool_call>"));
        assert!(result.iterations >= 2);
        assert!(result.response.contains("深圳") || result.response.contains("天气"));

...
async fn summarize_messages(
    ...
    if conversation_text.len() > effective_max {
        ...
        // Find valid char boundary at or after start (avoid slicing at invalid boundary)
        let safe_start = safe_char_boundary_from(&conversation_text, start);
...
/// Return a safe UTF-8 boundary at or after `idx` without slicing on invalid boundaries.
fn safe_char_boundary_from(text: &str, idx: usize) -> usize {

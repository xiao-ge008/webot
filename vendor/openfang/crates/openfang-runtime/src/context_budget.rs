...
pub fn truncate_tool_result_dynamic(content: &str, budget: &ContextBudget) -> String {
    ...
    let safe_cap = safe_char_boundary(content, cap);
    let search_start = safe_char_boundary(content, safe_cap.saturating_sub(200));
    ...
    let break_point = safe_char_boundary(content, break_point);
...
fn truncate_to(content: &str, max_chars: usize) -> String {
    ...
    let keep = safe_char_boundary(content, keep);
    let search_start = safe_char_boundary(content, keep.saturating_sub(100));
...
/// Return a safe UTF-8 boundary at or before `idx` without slicing on invalid boundaries.

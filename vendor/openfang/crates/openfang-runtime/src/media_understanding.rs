...
impl MediaEngine {
    ...
    pub async fn describe_image(
        ...
        if let MediaSource::FilePath { path } = &attachment.source {
            ...
            .await
            {
                ...
                Ok(None) => {}
                Err(err) => errors.push(format!("Local vision stack failed: {err}")),
            }
        }
    ...
    pub async fn describe_video(
        ...
        let mut errors = Vec::new();
        if let Some(understanding) = self
            .describe_video_from_local_derivatives(attachment)
            .await?
        {
            return Ok(understanding);
        }
        ...
        if !self.config.video_description {
            errors.push("Video description is disabled in configuration".to_string());
        } else if std::env::var("GEMINI_API_KEY").is_err()
            && std::env::var("GOOGLE_API_KEY").is_err()
        {
            errors.push("Video description requires GEMINI_API_KEY or GOOGLE_API_KEY".to_string());
        } else {
            return Ok(MediaUnderstanding {
        ...
        if errors.is_empty() {
            Err("No usable video understanding route is available".to_string())
        } else {
            Err(errors.join(" | "))
        }
    }
...
impl MediaEngine {
    async fn describe_video_from_local_derivatives(
        &self,
        attachment: &MediaAttachment,
    ) -> Result<Option<MediaUnderstanding>, String> {
        let source_path = match self.materialize_video_source_path(attachment).await? {
            Some(path) => path,
            None => return Ok(None),
        };
        let temp_dir =
            std::env::temp_dir().join(format!("openfang_video_analysis_{}", uuid::Uuid::new_v4()));
        tokio::fs::create_dir_all(&temp_dir)
            .await
            .map_err(|e| format!("Failed to create temp dir for video analysis: {e}"))?;
        let mut description_parts = Vec::new();
        let mut provider_parts = Vec::new();
        let mut model_parts = Vec::new();
        let mut errors = Vec::new();

        let frame_path = temp_dir.join("video_preview.png");
        match extract_video_frame(&source_path, &frame_path).await {
            Ok(()) => {
                let frame_size = match tokio::fs::metadata(&frame_path).await {
                    Ok(meta) => meta.len(),
                    Err(_) => 0,
                };
                ...
                match self.describe_image(&frame_attachment).await {
                    Ok(image_understanding) => {
                        description_parts
                            .push(format!("画面分析：{}", image_understanding.description));
                        provider_parts.push(format!("frame:{}", image_understanding.provider));
                        model_parts.push(format!("frame:{}", image_understanding.model));
                    }
                    Err(err) => errors.push(format!("Frame understanding failed: {err}")),
                }
            }
            Err(err) => errors.push(err),
        }

        let audio_path = temp_dir.join("video_audio.mp3");
        match extract_video_audio(&source_path, &audio_path).await {
            Ok(true) => {
                let audio_size = match tokio::fs::metadata(&audio_path).await {
                    Ok(meta) => meta.len(),
                    Err(_) => 0,
                };
                let audio_attachment = MediaAttachment {
                    media_type: MediaType::Audio,
                ...
                match self.transcribe_audio(&audio_attachment).await {
                    Ok(audio_understanding) => {
                        description_parts
                            .push(format!("音轨转写：{}", audio_understanding.description));
                        provider_parts.push(format!("audio:{}", audio_understanding.provider));
                        model_parts.push(format!("audio:{}", audio_understanding.model));
                    }
                    Err(err) => errors.push(format!("Audio understanding failed: {err}")),
                }
            }
            Ok(false) => {}
            Err(err) => errors.push(err),
        ...
        let provider = if provider_parts.is_empty() {
            "local_video_bridge".to_string()
        } else {
            provider_parts.join(" + ")
        };
        let model = if model_parts.is_empty() {
            "frame-audio-fallback".to_string()
        } else {
            model_parts.join(" + ")
        };
    ...
    async fn materialize_video_source_path(
        &self,
        attachment: &MediaAttachment,
    ) -> Result<Option<PathBuf>, String> {
        match &attachment.source {
            MediaSource::FilePath { path } => Ok(Some(PathBuf::from(path))),
            MediaSource::Base64 { data, .. } => {
                use base64::Engine;
                let output_path = std::env::temp_dir().join(format!(
                    "openfang_video_source_{}.mp4",
                    uuid::Uuid::new_v4()
                ));
                ...
                tokio::fs::write(&output_path, bytes)
                    .await
                    .map_err(|e| format!("Failed to materialize base64 video: {e}"))?;
                Ok(Some(output_path))
            }
            MediaSource::Url { .. } => Ok(None),
        }
    }
}

async fn extract_video_frame(source_path: &Path, output_path: &Path) -> Result<(), String> {
    ...
    let output = Command::new("ffmpeg")
        ...
        .output()
        .await
        .map_err(|e| format!("ffmpeg frame extraction unavailable: {e}"))?;
...
async fn extract_video_audio(source_path: &Path, output_path: &Path) -> Result<bool, String> {
    let input = source_path.to_string_lossy().to_string();
    let output_file = output_path.to_string_lossy().to_string();
    let output = Command::new("ffmpeg")
        .args([
        ...
        .output()
        .await
        .map_err(|e| format!("ffmpeg audio extraction unavailable: {e}"))?;
    if output.status.success() {

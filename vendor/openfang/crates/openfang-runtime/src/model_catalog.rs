...
impl ModelCatalog {
    ...
    /// Apply a batch of provider configurations from config.
    ///
    /// This is used to dynamically configure providers from config.toml.
    pub fn apply_provider_configs(&mut self, configs: &[ProviderConfigEntry]) {
        for cfg in configs {
            if let Some(ref base_url) = cfg.base_url {
                self.set_provider_url(&cfg.id, base_url);
            }
            if let Some(p) = self.providers.iter_mut().find(|p| p.id == cfg.id) {
                if cfg.api_key.is_some() || std::env::var(&p.api_key_env).is_ok() {
                    p.auth_status = AuthStatus::Configured;
                }
            }
        }

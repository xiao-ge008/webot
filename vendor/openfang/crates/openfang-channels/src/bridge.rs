...
pub trait ChannelBridgeHandle: Send + Sync {
    ...
    /// Send a message to an agent with channel conversation context.
    ///
    /// Default implementation falls back to `send_message` to preserve compatibility.
    async fn send_message_with_context(
        &self,
        agent_id: AgentId,
        message: &str,
        _context: &ChannelConversationContext,
...
/// Conversation scope context extracted from a channel message.
#[derive(Debug, Clone)]
pub struct ChannelConversationContext {
    /// Channel type key, e.g. "telegram", "discord".
    pub channel_type: String,
    /// Sender platform ID.
    pub sender_platform_id: String,
    /// Whether the source message is from a group context.
    pub is_group: bool,
    /// Best-effort group/conversation identifier from platform metadata.
    pub conversation_id: Option<String>,
    /// Optional thread identifier from the platform.
...
fn derive_conversation_context(message: &ChannelMessage) -> ChannelConversationContext {
    let channel_type = channel_type_str(&message.channel).to_string();
    ...
    .iter()
    .find_map(|k| metadata_str(&message.metadata, k))
    .or_else(|| {
        if message.is_group && !sender_platform_id.trim().is_empty() {
            Some(sender_platform_id.clone())
        } else {
            None
        }
    });
    ...
    let thread_id = message
        .thread_id
        .clone()
        .or_else(|| metadata_str(&message.metadata, "thread_id"))
        .or_else(|| metadata_str(&message.metadata, "threadId"))
        .or_else(|| metadata_str(&message.metadata, "thread_ts"));
...
async fn dispatch_message(
    ...
    let conversation_ctx = derive_conversation_context(message);
            ...
            match strategy {
                openfang_types::config::BroadcastStrategy::Parallel => {
                    ...
                    for (name, maybe_id) in &targets {
                        if let Some(aid) = maybe_id {
                            ...
                            let aid = *aid;
                            let name = name.clone();
                            let ctx = conversation_ctx.clone();
                            handles_vec.push(tokio::spawn(async move {
                                let result = h.send_message_with_context(aid, &t, &ctx).await;
                                (name, aid, result)
                            }));
                        }
                    }
                ...
                openfang_types::config::BroadcastStrategy::Sequential => {
                    for (name, maybe_id) in &targets {
                        if let Some(aid) = maybe_id {
                            match handle
                                .send_message_with_context(*aid, &text, &conversation_ctx)
                                .await
                            {
                                Ok(r) => responses.push(format!("[{name}]: {r}")),
                                Err(e) => responses.push(format!("[{name}]: Error: {e}")),
    ...
    match handle
        .send_message_with_context(agent_id, &text, &conversation_ctx)
        .await
    {
        Ok(response) => {

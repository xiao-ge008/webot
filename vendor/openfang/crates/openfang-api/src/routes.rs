...
/// GET /api/memory/agents/:id/search — Search semantic memories for an agent.
pub async fn search_agent_memories(
        ...
        .await
    {
        Ok(memories) => {
            let rows: Vec<serde_json::Value> = memories
                ...
                .map(|m| {
                    serde_json::json!({
                        "id": m.id.0.to_string(),
                        "agent_id": m.agent_id.0.to_string(),
                        "content": m.content,
                        "source": m.source,
                        "scope": m.scope,
        ...
        Err(e) => {
            tracing::warn!("Memory search failed for agent {id}: {e}");
            (
                StatusCode::INTERNAL_SERVER_ERROR,
...
/// GET /api/memory/agents/:id/items/:memory_id — Get a single memory item detail.
pub async fn get_agent_memory_item(
    ...
    let memory_uuid = match uuid::Uuid::parse_str(&memory_id) {
        Ok(v) => openfang_types::memory::MemoryId(v),
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
    ...
    match state
        .kernel
        .memory
        .get_memory_item_async(agent_id, memory_uuid)
        .await
    {
        Ok(Some(item)) => (StatusCode::OK, Json(serde_json::json!({ "memory": item }))),
        Ok(None) => (
            StatusCode::NOT_FOUND,
            Json(serde_json::json!({"error": "Memory not found"})),
        ),
        Err(e) => {
            tracing::warn!("Memory detail failed for {memory_id}: {e}");
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(serde_json::json!({"error": "Memory operation failed"})),
            )
...
/// POST /api/memory/agents/:id/feedback — Feedback/correction for a memory.
///
/// actions:
/// - confirm: increase confidence
/// - weaken: decrease confidence
/// - outdated: mark superseded
/// - revoke/delete/reject: revoke and hide from recall
/// - correct: create corrected replacement and supersede old memory
pub async fn feedback_agent_memory(
    ...
    let memory_uuid = match uuid::Uuid::parse_str(req.memory_id.trim()) {
        Ok(v) => openfang_types::memory::MemoryId(v),
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
        ...
        .await
    {
        Ok(Some(corrected_id)) => (
            ...
            Json(serde_json::json!({
                ...
                "action": req.action.trim().to_lowercase(),
        ...
        Ok(None) => (
            ...
            Json(serde_json::json!({
                ...
                "memory_id": req.memory_id,
                "action": req.action.trim().to_lowercase(),
                "corrected_memory_id": serde_json::Value::Null,
            })),
        ),
        Err(e) => {
            tracing::warn!("Memory feedback failed for agent {id}: {e}");
            let msg = format!("{e}");
            if msg.contains("Unsupported feedback action")
                || msg.contains("corrected_content is required")
            ...
            if msg.contains("Memory not found") {
                return (
                    StatusCode::NOT_FOUND,
                    Json(serde_json::json!({"error": "Memory not found"})),
                );
            ...
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(serde_json::json!({"error": "Memory feedback failed"})),
            )
        }
    }
}

/// DELETE /api/memory/agents/:id/items/:memory_id — Revoke a memory item.
pub async fn delete_agent_memory_item(
    ...
    let memory_uuid = match uuid::Uuid::parse_str(memory_id.trim()) {
        Ok(v) => openfang_types::memory::MemoryId(v),
        Err(_) => {
            return (
                StatusCode::BAD_REQUEST,
    ...
    match state
        .kernel
        .memory
        .feedback_memory_async(agent_id, memory_uuid, "revoke", None, None)
        .await
    {
        Ok(_) => (
            StatusCode::OK,
            Json(serde_json::json!({"status": "revoked", "memory_id": memory_id})),
        ),
        Err(e) => {
            tracing::warn!("Memory revoke failed: {e}");
            if format!("{e}").contains("Memory not found") {
                return (
                    StatusCode::NOT_FOUND,
                    Json(serde_json::json!({"error": "Memory not found"})),
                );
...
pub async fn list_cron_jobs(
    ...
    let jobs_json: Vec<serde_json::Value> = jobs
        .into_iter()
        .map(|job| {
            let meta = state.kernel.cron_scheduler.get_meta(job.id);

...
use dashmap::DashMap;
use openfang_channels::bridge::{BridgeManager, ChannelBridgeHandle, ChannelConversationContext};
...
pub struct KernelBridgeAdapter {
    ...
    started_at: Instant,
    session_locks: DashMap<AgentId, Arc<Mutex<()>>>,
}

const GROUP_SESSION_LABEL_PREFIX: &str = "groupmem";

fn normalize_label_component(raw: &str, max_len: usize) -> String {
...
fn build_group_session_label(context: &ChannelConversationContext) -> Option<String> {
    if !context.is_group {
        return None;
    }
    let base_conversation = context
        .conversation_id
        .as_deref()
        .unwrap_or(&context.sender_platform_id);
    if base_conversation.trim().is_empty() {
        return None;
    }

    let channel = normalize_label_component(&context.channel_type, 24);
    ...
    // Keep labels comfortably below the 128-char UI/session-label constraints.
    if label.len() > 120 {
        let mut hasher = std::collections::hash_map::DefaultHasher::new();
        label.hash(&mut hasher);
        let digest = format!("{:016x}", hasher.finish());
        let prefix: String = label.chars().take(96).collect();
        label = format!("{prefix}_{digest}");
    }

    Some(label)
}

fn build_participant_scope(group_label: &str, sender_platform_id: &str) -> Option<String> {
    let normalized_sender = normalize_label_component(sender_platform_id, 48);
    if normalized_sender == "na" {
        return None;
    }
    Some(format!("{group_label}::user::{normalized_sender}"))
}

impl KernelBridgeAdapter {
    fn session_lock(&self, agent_id: AgentId) -> Arc<Mutex<()>> {
        self.session_locks
            .entry(agent_id)
    ...
    async fn send_message_in_group_session(
        &self,
        agent_id: AgentId,
        message: &str,
        label: &str,
        ...
        let lock = self.session_lock(agent_id);
        ...
        let target_session = match self
            ...
            .map_err(|e| format!("Failed to lookup group session: {e}"))?
        {
        ...
        let switched = target_session != original_session_id;
        if switched {
            self.kernel
                .registry
                .update_session_id(agent_id, target_session)
                .map_err(|e| format!("Failed to switch to group session: {e}"))?;
        }

        let memory_turn_context = Some(openfang_runtime::agent_loop::MemoryTurnContext {
            conversation_scope: Some(label.to_string()),
            participant_scope: build_participant_scope(label, &context.sender_platform_id),
        });
        let run_result = self
            .kernel
            .send_message_with_memory_context(agent_id, message, memory_turn_context)
            .await;
        ...
        if switched {
            if let Err(e) = self
                .kernel
                .registry
                .update_session_id(agent_id, original_session_id)
            {
                warn!(
                    agent_id = %agent_id,
                    label = %label,
                    "Failed to restore original session after group message: {e}"
                );
            }
        }
...
impl ChannelBridgeHandle for KernelBridgeAdapter {
    ...
    async fn send_message_with_context(
        ...
        if let Some(label) = build_group_session_label(context) {
            return self
                .send_message_in_group_session(agent_id, message, &label, context)
                .await;
        }

        ...
        "message" => {
            ...
            if let Some(attachments) = parsed["attachments"].as_array() {
                let refs: Vec<crate::types::AttachmentRef> = attachments
                    ...
                    .collect();
                if !refs.is_empty() {
                    let image_blocks = crate::routes::resolve_image_attachments(&refs);
                ...
                Ok((mut rx, handle)) => {
                    ...
                    match handle.await {
                        ...
                        Ok(Err(e)) => {
                            ...
                            .await;
                            let mut user_msg = classify_streaming_error(&e);
                            let raw = e.to_string();
                            let raw_trim = raw.trim();
                            if !raw_trim.is_empty() && !user_msg.contains(raw_trim) {
                                let suffix = truncate_message(raw_trim, 600);
                                user_msg = format!("{user_msg} [raw: {suffix}]");
                            }
                            let _ = send_json(
                                sender,
                                &serde_json::json!({
...
/// Truncate long diagnostic text to keep UI readable.

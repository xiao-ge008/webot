...
/// Maximum retained run logs per job.
const MAX_RECENT_RUNS: usize = 200;

/// Lightweight run record for chat/task progress replay.
...
pub struct JobMeta {
    ...
    /// Total executed runs (success + failure).
    #[serde(default)]
    pub run_count: u64,
    /// Latest human-readable output summary.
    #[serde(default)]
    pub last_output: Option<String>,
    /// Recent run records for status/progress UI.
    #[serde(default)]
...
impl JobMeta {
    ...
    fn push_run(&mut self, kind: &str, message: &str) {
        let trimmed = message.trim();
        if trimmed.is_empty() {
            return;
        }
        self.runs.push(JobRunRecord {
            event_id: uuid::Uuid::new_v4().to_string(),
            created_at: Utc::now(),
            kind: kind.to_string(),
            message: openfang_types::truncate_str(trimmed, 3000).to_string(),
        });
        if self.runs.len() > MAX_RECENT_RUNS {
            let overflow = self.runs.len() - MAX_RECENT_RUNS;
...
impl CronScheduler {
    ...
    pub fn record_success(&self, id: CronJobId, output_summary: Option<&str>) {
        ...
        let should_remove = {
            if let Some(mut meta) = self.jobs.get_mut(&id) {
                ...
                meta.consecutive_errors = 0;
                meta.run_count = meta.run_count.saturating_add(1);
                let summary = output_summary
                    .map(str::trim)
                    .filter(|text| !text.is_empty())
                    .unwrap_or("completed");
                meta.last_output = Some(openfang_types::truncate_str(summary, 3000).to_string());
                meta.push_run("ok", summary);
    ...
    pub fn record_failure(&self, id: CronJobId, error_msg: &str) {
        if let Some(mut meta) = self.jobs.get_mut(&id) {
            ...
            meta.run_count = meta.run_count.saturating_add(1);
            let summary = openfang_types::truncate_str(error_msg.trim(), 3000).to_string();
            meta.last_output = Some(summary.clone());
            meta.push_run("error", &summary);
...
mod tests {
    ...
    fn test_record_success_removes_one_shot() {
        ...
        sched.record_success(id, Some("ok"));
    ...
    fn test_record_success_keeps_recurring() {
        ...
        assert_eq!(meta.last_output.as_deref(), Some("ok"));
        assert_eq!(meta.runs.len(), 1);
        assert!(meta.job.last_run.is_some());
    }

    #[test]
    fn test_record_success_tracks_output_summary() {
        let (sched, _tmp) = make_scheduler(100);
        let agent = AgentId::new();
        let job = make_job(agent);
        let id = sched.add_job(job, false).unwrap();

        sched.record_success(id, Some("BTC: 69123.10"));
        sched.record_success(id, Some("BTC: 69201.55"));

        let meta = sched.get_meta(id).unwrap();
        assert_eq!(meta.run_count, 2);
        assert_eq!(meta.last_status.as_deref(), Some("ok"));
        assert_eq!(meta.last_output.as_deref(), Some("BTC: 69201.55"));
        assert_eq!(meta.runs.len(), 2);
        assert_eq!(meta.runs[0].kind, "ok");
        assert_eq!(meta.runs[1].kind, "ok");
    }
    ...
    fn test_record_failure_auto_disable() {
        ...
        assert_eq!(meta.run_count, MAX_CONSECUTIVE_ERRORS as u64);
        assert_eq!(meta.runs.len(), MAX_CONSECUTIVE_ERRORS as usize);
        assert_eq!(meta.runs.last().map(|r| r.kind.as_str()), Some("error"));
    }

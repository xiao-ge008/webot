...
impl OpenFangKernel {
        ...
        let driver_config = {
            ...
            DriverConfig {
                ...
                base_url: config
                    ...
                    .base_url
                    .clone()
                    .or_else(|| p_cfg.and_then(|p| p.base_url.clone()))
    ...
    ///
    /// Used by channel bridge to attach group/user memory scopes without affecting
    /// normal callers.
    pub async fn send_message_with_memory_context(
        &self,
        agent_id: AgentId,
        message: &str,
    ...
    pub fn send_message_streaming_with_quota_scope(
        ...
        // Build the structured system prompt via prompt_builder
        {
            ...
            let prompt_ctx = openfang_runtime::prompt_builder::PromptContext {
                ...
                canonical_context: if use_canonical_context {
                    self.memory
                ...
                agents_md: read_identity_file(&self.config.home_dir, "AGENTS.md"),
        ...
        let handle = tokio::spawn(async move {
            ...
            match result {
                Ok(result) => {
                    ...
                    if use_canonical_context && session.messages.len() > messages_before {
        ...
        let primary = if agent_provider == default_provider && !has_custom_key && !has_custom_url {
            ...
            let api_key = if has_custom_key {
                ...
                if let Some(profiles) = self.config.auth_profiles.get(agent_provider.as_str()) {
                    ...
                    provider_cfg
                        .and_then(|p| p.api_key.clone())
                        .or_else(|| std::env::var(provider_env_key(agent_provider)).ok())
                }
            };
    ...
    fn available_tools(&self, agent_id: AgentId) -> Vec<ToolDefinition> {
        ...
        let has_any_tool_cap = caps
            .iter()
            .any(|c| matches!(c, Capability::ToolAll | Capability::ToolInvoke(_)));

        // Compatibility fallback:
        // Some legacy agents were persisted with an empty capability set,
        // causing text `<tool_call>...` to never execute.
        // If no tool capability is present and manifest has no explicit tool list,
        // fall back to the current visible tool set (already filtered by
        // profile/allowlist/blocklist/exec-policy above).
        if !has_any_tool_cap {
            if let Some(e) = entry.as_ref() {
                if e.manifest.capabilities.tools.is_empty() {
                    warn!(
                        agent = %e.manifest.name,
                        "Agent has no tool capabilities; applying visible-tool fallback"
                    );
                    return all_tools;
                }
            }
...
/// from the manifest.
fn is_manifest_capabilities_fully_empty(caps: &ManifestCapabilities) -> bool {
    caps.tools.is_empty()
        && caps.network.is_empty()
        && caps.shell.is_empty()
        && !caps.agent_spawn
        && caps.agent_message.is_empty()
...
fn apply_manifest_non_tool_overrides(
    base: &mut ManifestCapabilities,
...
fn manifest_to_capabilities(manifest: &AgentManifest) -> Vec<Capability> {
    ...
    let effective_caps = if let Some(ref profile) = manifest.profile {
        if manifest.capabilities.tools.is_empty() {
            ...
            apply_manifest_non_tool_overrides(&mut merged, &manifest.capabilities);
            merged
        } else {
            manifest.capabilities.clone()
        }
    } else if manifest.capabilities.tools.is_empty()
        && is_manifest_capabilities_fully_empty(&manifest.capabilities)
    {
        // Backward compatibility:
        // Legacy manifests created without `profile` and without any explicit
        // capability fields ended up with no tools at all.
        // Treat fully-empty capability blocks as `profile = "full"`.
        ToolProfile::Full.implied_capabilities()
    } else {
        manifest.capabilities.clone()
    };
...
fn should_use_canonical_context_for_label(label: Option<&str>) -> bool {
    ...
    !label
        .map(str::trim)
        .is_some_and(|v| v.starts_with("groupmem_"))
}
...
mod tests {
    ...
    #[test]
    fn test_manifest_to_capabilities_legacy_empty_defaults_to_full_profile() {
        let manifest = AgentManifest {
            profile: None,
            capabilities: ManifestCapabilities::default(),
            ..Default::default()
        ...
        assert!(caps
            .iter()
            .any(|c| matches!(c, Capability::ToolInvoke(name) if name == "*")));
        assert!(caps
            .iter()
            .any(|c| matches!(c, Capability::NetConnect(host) if host == "*")));
    }

    #[test]
    fn test_manifest_to_capabilities_partial_explicit_keeps_no_tool_fallback() {
        let mut manifest = AgentManifest {
            profile: None,
            capabilities: ManifestCapabilities::default(),
            ..Default::default()
        };
        manifest.capabilities.network = vec!["example.com:443".to_string()];

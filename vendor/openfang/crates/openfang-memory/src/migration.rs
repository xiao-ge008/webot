...
/// Version 8: Add orchestration columns for typed memory, lifecycle, and conflict handling.
fn migrate_v8(conn: &Connection) -> Result<(), rusqlite::Error> {
    if !column_exists(conn, "memories", "memory_type") {
        conn.execute(
            "ALTER TABLE memories ADD COLUMN memory_type TEXT NOT NULL DEFAULT 'episodic'",
            [],
        )?;
    }
    if !column_exists(conn, "memories", "importance") {
        conn.execute(
            "ALTER TABLE memories ADD COLUMN importance REAL NOT NULL DEFAULT 0.5",
            [],
        )?;
    }
    if !column_exists(conn, "memories", "entity_key") {
        conn.execute("ALTER TABLE memories ADD COLUMN entity_key TEXT", [])?;
    }
    ...
    if !column_exists(conn, "memories", "supersedes_id") {
    ...
    conn.execute_batch(
        "
        CREATE INDEX IF NOT EXISTS idx_memories_agent_type_status ON memories(agent_id, memory_type, status);
        CREATE INDEX IF NOT EXISTS idx_memories_entity_key ON memories(entity_key);
        CREATE INDEX IF NOT EXISTS idx_memories_expires_at ON memories(expires_at);

        INSERT OR IGNORE INTO migrations (version, applied_at, description)
        VALUES (8, datetime('now'), 'Add typed-memory lifecycle and conflict columns');
        ",
    )?;
...
/// Version 9: Add memory access and feedback audit tables.
fn migrate_v9(conn: &Connection) -> Result<(), rusqlite::Error> {
    conn.execute_batch(
        ...
        CREATE INDEX IF NOT EXISTS idx_memory_access_agent_time ON memory_access_log(agent_id, created_at);
        CREATE INDEX IF NOT EXISTS idx_memory_access_memory_id ON memory_access_log(memory_id);
        ...
        CREATE INDEX IF NOT EXISTS idx_memory_feedback_agent_time ON memory_feedback_events(agent_id, created_at);
        CREATE INDEX IF NOT EXISTS idx_memory_feedback_memory_id ON memory_feedback_events(memory_id);

        INSERT OR IGNORE INTO migrations (version, applied_at, description)
        VALUES (9, datetime('now'), 'Add memory access/feedback audit tables');
        ",
    )?;
